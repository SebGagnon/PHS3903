...
def animer_resultats(
    ...
    if len(signaux_capteurs) > 0:
        amp_max = max(np.max(np.abs(signal)) for signal in signaux_capteurs)
    else:
        amp_max = 0.0
...
# objet mobile
objet = {
    "forme": "rectangle",              # "cercle" ou "rectangle"
    "centre_init_m": (12.0, 25.0),  # position initiale en mètres
    "vitesse_m_s": (200.0, -30.0),     # vitesse (vx, vy) en m/s
    # "rayon_m": 2.0,                 # pour un cercle
    "taille_m": (4.0, 2.0),       # pour un rectangle
