...
def estime_vitesse(positions_estimees, t_impacts, objet, afficher=True):
    ...
    plt.show()
    return vitesse, (x0_est, y0_est)
def borne_capteur(x_m, y_m, taille, ratio_pml, marge_m=5.0):
    '''Force un capteur à rester hors du PML.'''
    debut_zone = ratio_pml * taille + marge_m
    fin_zone = (1.0 - ratio_pml) * taille - marge_m

    x_m = float(np.clip(x_m, debut_zone, fin_zone))
    y_m = float(np.clip(y_m, debut_zone, fin_zone))
    return x_m, y_m
def capteurs_depuis_vecteur(x, taille, ratio_pml, t_emissions):
    '''Construit les capteurs à partir d un vecteur [x1,y1,x2,y2,...].'''
    nb_capteurs = len(t_emissions)

    if len(x) != 2 * nb_capteurs:
        raise ValueError('Le vecteur x doit contenir 2 coordonnées par capteur.')
...
def objet_loin():
    '''Objet factice pour la simulation sans bateau.'''
    return {
        'forme': 'cercle',
        'centre_init_m': (0.0, 0.0),
        'vitesse_m_s': (0.0, 0.0),
...
def erreur_moyenne_multilateration(params, x_capteurs, objet, t_emissions):
    '''Retourne l erreur moyenne de position avec la multilateration.'''
    capteurs = capteurs_depuis_vecteur(
        x_capteurs,
        taille=params['taille'],
        ratio_pml=params['ratio_pml'],
    ...
    for idx_emit, t_debut, t_fin in plages_pulses(capteurs, t_total):
        try:
            pos_est, temps_echo, distances, idx_retour = multilateration_un_pulse(
                intensite_echo,
                temps,
            ...
            err = np.linalg.norm(pos_est - pos_reelle)
    ...
    return {
        'score': float(np.mean(erreurs)),
        'erreurs': erreurs,
        'capteurs': capteurs,
        'positions_estimees': positions_estimees,
        'positions_reelles': positions_reelles,
    }
def visualiser_setup_multilateration(params, x_capteurs, objet, t_emissions):
    '''Affiche les simulations avec et sans objet pour valider le setup.'''
    capteurs = capteurs_depuis_vecteur(
        x_capteurs,
        taille=params['taille'],
        ratio_pml=params['ratio_pml'],
    ...
    print('Capteurs utilisés :')
    for capteur in capteurs:
        print(capteur)

    print('\nSimulation sans objet...')
    res_sans_objet = run_simul(
        **base,
        objet=objet_loin(),
    )

    print('\nSimulation avec objet...')
    res_avec_objet = run_simul(
        **base,
        objet=objet,
    )
    ...
    return {
        'capteurs': capteurs,
        'res_sans_objet': res_sans_objet,
        'res_avec_objet': res_avec_objet,
    }
def visualiser_methode_multilateration(params, x_capteurs, objet, t_emissions, afficher_echos=True):
    '''Affiche la simulation et toute la méthode de multilateration.'''
    capteurs = capteurs_depuis_vecteur(
        x_capteurs,
        taille=params['taille'],
        ratio_pml=params['ratio_pml'],
    ...
    return methode_multilateration(
        params=params,
        capteurs=capteurs,
        objet=objet,
        afficher_echos=afficher_echos,
    )
def distance_min_capteurs(x_capteurs):
    '''Retourne la distance minimale entre deux capteurs.'''
    pts = np.array(x_capteurs, dtype=float).reshape(-1, 2)
    ...
    return d_min
def optimise_positions_capteurs_bo(
    params,
    t_emissions,
    n_calls=20,
    n_initial_points=6,
    ...
    '''Optimise les positions des capteurs par BO avec plusieurs objets aléatoires par essai.'''
    nb_capteurs = len(t_emissions)

    borne_inf = params['ratio_pml'] * params['taille'] + marge_m
    borne_sup = (1.0 - params['ratio_pml']) * params['taille'] - marge_m

    espace = []
    for i in range(nb_capteurs):
        espace.append(Real(borne_inf, borne_sup, name=f'x{i+1}'))
        espace.append(Real(borne_inf, borne_sup, name=f'y{i+1}'))
    ...
    def objectif(**kwargs):
        ...
        x_capteurs = []
        for i in range(nb_capteurs):
            x_capteurs.append(kwargs[f'x{i+1}'])
            x_capteurs.append(kwargs[f'y{i+1}'])

        print('\n' + '=' * 70)
        print(f'Évaluation BO {idx_eval}/{n_calls}')
        print(f'Positions proposées = {np.round(x_capteurs, 1)}')

        d_min = distance_min_capteurs(x_capteurs)
        print(f'Distance minimale entre capteurs = {d_min:.2f} m')

        if d_min < dist_min_m:
            score = 1e4 + 100 * (dist_min_m - d_min)
            capteurs = capteurs_depuis_vecteur(
                x_capteurs,
                taille=params['taille'],
                ratio_pml=params['ratio_pml'],
                ...
                pickle.dump(
                    {
                        ...
                        'n_tests_par_eval': n_tests_par_eval,
                    },
                    f,
                )

            print(f'Configuration refusée: capteurs trop proches | score pénalité = {score:.3f}')
            return score

        out = erreur_moyenne_multilateration_randomisee(
            params=params,
            x_capteurs=x_capteurs,
            t_emissions=t_emissions,
            n_tests=n_tests_par_eval,
            vitesse_objet=vitesse_objet,
            rayon_objet=rayon_objet,
            c_objet=c_objet,
            gamma_objet=gamma_objet,
            marge_objet_m=marge_objet_m,
        ...
        entree = {
            ...
            'distance_min': d_min,
            'valide': True,
            'scores_individuels': out['scores'],
            'details_tests': out['details'],
        }
        historique.append(entree)
        ...
        meilleur = min(h['score'] for h in historique if np.isfinite(h['score']))
        print(f'Score moyen de cette évaluation = {score:.4f}')
        print(f'Meilleur score jusqu ici = {meilleur:.4f}')
        print('=' * 70)
    ...
    print(f'Meilleur score = {res_bo.fun:.4f}')
    print(f'Meilleures positions = {np.round(res_bo.x, 2)}')
    print('#' * 70)
...
def objet_aleatoire(
    ...
    '''Crée un objet aléatoire qui reste dans le domaine utile pendant tout le test.'''
    if rng is None:
        rng = np.random.default_rng()

    # zone utile: hors du PML, avec une marge supplémentaire
    borne_inf = ratio_pml * taille + rayon_objet + marge_m
    borne_sup = (1.0 - ratio_pml) * taille - rayon_objet - marge_m

    if borne_inf >= borne_sup:
        raise ValueError('La zone utile est vide. Réduis rayon_objet ou marge_m.')

    for _ in range(1000):
        theta = rng.uniform(0.0, 2.0 * np.pi)
        vx = vitesse_objet * np.cos(theta)
        vy = vitesse_objet * np.sin(theta)

        dx = vx * t_total
        dy = vy * t_total

        # il faut que (x0, y0) ET (x0+dx, y0+dy) restent dans la zone utile
        x_min = max(borne_inf, borne_inf - dx)
        x_max = min(borne_sup, borne_sup - dx)

        y_min = max(borne_inf, borne_inf - dy)
        y_max = min(borne_sup, borne_sup - dy)
    ...
    raise ValueError(
        'Impossible de générer un objet qui reste dans le domaine. '
        'Réduis vitesse_objet, t_total, rayon_objet, ou marge_m.'
    )
def erreur_moyenne_multilateration_randomisee(
    params,
    x_capteurs,
    t_emissions,
    n_tests=5,
    ...
    '''Erreur moyenne sur plusieurs objets aléatoires.'''
    ...
    for k in range(n_tests):
        ...
        if verbose:
            ...
            print(
                f'    test {k+1}/{n_tests} | '
                f'centre=({x0:.1f}, {y0:.1f}) | '
                f'v=({vx:.1f}, {vy:.1f})'
            )

        out = erreur_moyenne_multilateration(
            params=params,
            x_capteurs=x_capteurs,
            objet=objet,
            t_emissions=t_emissions,
        ...
        if verbose:
            print(f'      score test = {out["score"]:.4f}')
    ...
    if np.all(~np.isfinite(scores)):
        ...
        score_moyen = float(np.mean(scores[np.isfinite(scores)]))
...
t_emissions = [0.1, 1.0, 1.75]

res_bo = optimise_positions_capteurs_bo(
    params=params_test,
    t_emissions=t_emissions,
    n_calls=25,
    n_initial_points=10,
    random_state=42,
    dist_min_m=50.0,
    save_path='bo_positions_capteurs_test3.pkl',
    n_tests_par_eval=30,
    vitesse_objet=50.0,
    rayon_objet=10.0,
)
...
print('score =', res_bo.fun)
print('x_opt =', res_bo.x)

capteurs_opt = capteurs_depuis_vecteur(
    res_bo.x,
    taille=params_test['taille'],
    ratio_pml=params_test['ratio_pml'],
    t_emissions=t_emissions,
)

print('\nCapteurs optimaux :')
for capteur in capteurs_opt:
    print(capteur)